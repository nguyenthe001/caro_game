...
{
	std::ofstream file("data/replays.txt", std::ios::app);
	if (!file) {
...
{
	std::ofstream file("data/replaysInfo.txt", std::ios::app);
	if (!file) {
		std::cerr << "Can not open file!\n";
	}
	file << str + end;
...
void Controller::replay()
{
	...
	std::ifstream input2("data/replaysInfo.txt");
	if (!input2) {
